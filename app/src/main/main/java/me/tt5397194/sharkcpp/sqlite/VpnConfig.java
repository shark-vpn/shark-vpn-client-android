...
public class VpnConfig {
    ...
    public static List<VpnConfig> GetAll() {
        ...
        String[] projection = {"_id", "name", "server_addr", "server_port", "password", "local_ip", "dns_addr", "mtu"};
        ...
        while (cursor.moveToNext()) {
            ...
            cfg.mtu = cursor.getInt(cursor.getColumnIndexOrThrow("mtu"));
            cfgLst.add(cfg);
        }
        cursor.close();
        return cfgLst;
    ...
    public static boolean Update(VpnConfig cfg) {
        String sql = "update vpn_config set name=?, server_addr=?, server_port=?, password=?, local_ip=?, dns_addr=?, mtu=? where _id=?";
        SQLiteDatabase db = MainActivity.dbHelper.getWritableDatabase();
        Object[] pras = new Object[]{cfg.vpn_name, cfg.server_addr, cfg.server_port, cfg.password, cfg.local_ip, cfg.dns_addr, cfg.mtu, cfg.id};
        db.execSQL(sql, pras);
        Log.e("zzz", sql);
        return true;
    }
